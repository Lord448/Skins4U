            ...
            try
            {
                ...
                if (Advertencia.variables.comprobador == "aceptar")
                {
                    ...
                    if (saldo_pre > PrecioRP)
                    {
                        ...
                        var insercion = new MySqlCommand("INSERT INTO Pagos VALUES ('" + ID + "', '" + Producto + "', '" + PrecioRP + "', '" + PrecioMXN + "', '" + Comprador + "', '" + Invocador + "', '" + plataforma + "', 'No entregado', '" + fecha + "', '" + vendedor + "', 'false')", conexion);
                        ...
                        if (saldo_post < 250)
                        {
                            var elim = new MySqlCommand("UPDATE Cuentas SET Estado='Inactivo' WHERE Nick='" + vendedor + "'", conexion);
                            elim.ExecuteNonQuery();
                            lblEstados.ForeColor = Color.Black;
                            lblEstados.Text = "Se añadió el producto con éxito, saldo actual: " + saldo_post.ToString() +"; por lo que se mandará a inactivo";
                        }
                    }
